...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '1RD2V,FD', options ) );
	}
	...
	if ( hasOwnProp( options, 'series' ) ) {
		...
		if ( !isBoolean( opts.series ) ) {
			return new TypeError( format( '1RD2o,GE', 'series', opts.series ) );
		}
	}
	if ( hasOwnProp( options, 'limit' ) ) {
		opts.limit = options.limit;
		if ( !isPositiveInteger( opts.limit ) ) {
			return new TypeError( format( '1RD3P,Fv', 'limit', opts.limit ) );
		}
	}
	if ( hasOwnProp( options, 'returns' ) ) {
		opts.returns = options.returns;
		if ( indexOf( returns, opts.returns ) === -1 ) {
			return new TypeError( format( '1RD2X,3g,4S,GD,Gg,Jm', 'returns', returns.join( '", "' ), opts.returns ) );
		}
	}
	return null;
}
